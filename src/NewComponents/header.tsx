...
const Header: React.FC<{
  ...
  //   const { bigLogo, heartLogo } = useStaticQuery<{
  //     bigLogo: FilePublicUrl
  //     heartLogo: FilePublicUrl
  //   }>(graphql`
  //     query {
  ...
  const menuButtonClassName = `text-center text-white hamburger ${menuClassName} ${
    !isTop && 'hamburger-below'
  }`
  return (
    <nav
      className={`navbar navbar-expand-lg fixed-top text-center ${
        isTop || menuOpen ? 'navbar-trans' : 'bg-lights'
      }`}
      ref={forwardRef}
    >
      <div className="brand">
        <div className={menuOpen ? 'hide-logo' : 'show-logo'}>
          <Link id="ucare-logo" to="/">
            <img src={ucareLogo} alt="Logo" />
          </Link>
          <Link id="ucare-heart" to="/">
            <img src={ucareHeart} alt="Logo" />
          </Link>
        </div>
      </div>
      ...
      <TrialLink
        className={`btn trial-btn ${isTop || menuOpen ? 'trial-hide' : ''}`}
      />
      ...
      <div
        className={`sidebar ${
          menuOpen ? 'sidebar-active' : 'sidebar-inactive'
        }`}
      >
