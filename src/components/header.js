				...
				render={data => {
						...
						<InView as='nav' onChange={inView => this.handleIntersection(inView)}>
							<nav
								className={`navbar navbar-expand-lg fixed-top text-center ${
									this.state.isTop ? 'navbar-trans' : 'bg-lights'
								}`}
							>
								<div className={this.state.isTop ? `biglogo` : `heartlogo`}>
									<div className={this.state.menuOpen ? `hide-logo` : `show-logo`}>
										<Link to='/'>
											<img src={this.state.isTop ? data.bigLogo.publicURL : data.heartLogo.publicURL} alt='Logo' />
										</Link>
									</div>
								</div>
									...
									{matches =>
										matches && (
											<ul className='navbar-nav mx-auto'>
												...
												<li>
													<a
														href='https://help.ucareapp.com/hc/en-us'
														className={`nav-item  ${!this.state.isTop && `item-down`}`}
													>
														support
													</a>
												...
												<li>
													<a
														href='https://connect.ucareapp.com/signin'
														className={`nav-item  ${!this.state.isTop && `item-down`}`}
													>
														sign in
													</a>
								...
								<Link className={`btn trial-btn ${this.state.isTop ? 'trial-hide' : ''}`} to='/sign-up'>
									free 30-day trial&nbsp;&nbsp;&nbsp;
									<FontAwesomeIcon icon={faChevronRight} />
								</Link>
								<button
									className={`text-center text-white hamburger 
                    ...
                    ${!this.state.isTop && `hamburger-below`}`}
								...
								<div className={`sidebar ${this.state.menuOpen ? `sidebar-active` : `sidebar-inactive`}`}>
									<Link to={`/features`} className={`sidebar-item`}>
										Features
									</Link>
									{this.state.location === '/' ? (
										<ScrollLink
									...
									<a href='https://ucare.zendesk.com/hc/en-us' className={`sidebar-item`}>
										support
									</a>
