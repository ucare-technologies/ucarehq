		...
		return (
			<Layout>
				...
				{posts.map(({ node }) => {
					...
					const { excerpt } = node;
					return (
						<BlogListTemp title={title} date={date} url={slug} excerpt={excerpt} imageURL={publicURL} key={slug} />
					);
				})}
					...
					<div className='arrow'>
						{!isLast && (
							<Link to={nextPage} rel='next'>
								<button type='button' className='btn btn-default arrow-btn' onClick={this.handleNext}>
