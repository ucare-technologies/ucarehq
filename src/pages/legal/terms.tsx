...
const TermsPage: React.FC<PageProps<Queries.TermsPageQuery>> = ({ data }) => {
	return (
		<Layout>
			<main>
...
export const Head: HeadFC<Queries.TermsPageQuery> = ({ data }) => (
	<HeadTags title={data.page?.page_title || 'Terms & Conditions'} />
);

function isBannerSection(item: Slice): item is Queries.LegalBannerSectionFragment {
	return item.slice_name === 'terms_banner';
}
...
export const pageQuery = graphql`
	query TermsPage {
		page: contentfulPage(slug: { eq: "/legal/terms" }) {
			page_name
			page_title
			sections {
