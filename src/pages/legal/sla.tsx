...
const SlaPage: React.FC<PageProps<Queries.SlaPageQuery>> = ({ data }) => (
	<Layout>
		<main>
...
export const Head: HeadFC<Queries.SlaPageQuery> = ({ data }) => (
	<HeadTags title={data.page?.page_title || 'Service Level Agreement (SLA)'} />
);

function isBannerSection(item: Slice): item is Queries.LegalBannerSectionFragment {
	return item.slice_name === 'sla_banner';
}
...
export const pageQuery = graphql`
	query SlaPage {
		page: contentfulPage(slug: { eq: "/legal/sla" }) {
			page_name
			page_title
			sections {
